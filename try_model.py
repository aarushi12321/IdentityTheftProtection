...
def get_wide_data(x_train_wide, x_test_wide):
    train_wide_list = []
    test_wide_list = []

    for i in tqdm(range(x_train_wide.shape[0])):
        sample = x_train_wide[i].reshape(int(x_train_wide.shape[1]/7), 7)
        train_wide_list.append(sample)

    for i in tqdm(range(x_test_wide.shape[0])):    
        sample = x_test_wide[i].reshape(int(x_test_wide.shape[1]/7), 7)
        test_wide_list.append(sample)

    return np.array(train_wide_list), np.array(test_wide_list)
...
def Wide_CNN(weeks, days, channel, wide_len, lr=0.005, decay=1e-5, momentum=0.9):
    ...
    inputs_wide_l = Input(shape=(wide_len,7))
    ...
    x_wide = LSTM(units=128, input_shape=(wide_len,7))(inputs_wide_l)
    x_wide = Dropout(0.2)(x_wide)
    x_wide = Flatten()(x_wide)
    x_wide = Dense(128, activation='relu')(x_wide)
    ...
    model = Model(inputs=[inputs_wide_l, inputs_deep], outputs=pred)
...
print('Defining data for lstm network...')
print('')
x_train_wide_l, x_test_wide_l = get_wide_data(x_train_wide, x_test_wide)

wide_len = x_train_wide_l.shape[1]
...
for i in tqdm(range(1), desc='Rounds'):
    ...
    model.fit([x_train_wide_l, x_train_pre], y_train, batch_size=64, epochs=35, verbose=1,
              validation_data=([x_test_wide_l, x_test_pre], y_test))
              # callbacks=[MyMetric(i, 0.8, ([x_test_wide, x_test_pre], y_test))]

pickle.dump(model, open('model_try.pkl','wb'))
